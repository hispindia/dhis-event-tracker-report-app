...
const SQLQUERY_EVENT= "select ps.uid psuid,min(ps.name) psname,psi.uid ev ,psi.executiondate evdate,de.uid deuid,\
min(de.name) dename,min(tedv.value) devalue,ou.name,\
psi.executiondate::DATE,ou.uid,ou.organisationunitid,ou.code,teav.value from programstageinstance psi\
INNER JOIN programinstance pi ON  psi.programinstanceid = pi.programinstanceid \
INNER JOIN trackedentityattributevalue teav ON  teav.trackedentityinstanceid = pi.trackedentityinstanceid\
INNER JOIN trackedentitydatavalue tedv ON tedv.programstageinstanceid = psi.programstageinstanceid \
INNER JOIN dataelement de ON de.dataelementid = tedv.dataelementid \
...
INNER JOIN organisationunit ou ON ou.organisationunitid = psi.organisationunitid \
WHERE psi.programstageid IN (select programstageid from programstage \
where programid IN (select programid from program where uid = '${program}')) \
and psi.organisationunitid IN (select organisationunitid from organisationunit \
where path like '%${orgunit}%') and psi.executiondate between '${startdate}' and '${enddate}' and teav.trackedentityattributeid=38565533\
 group by ps.uid,psi.uid,psi.executiondate,de.uid,ou.name, psi.executiondate,ou.uid,ou.organisationunitid,ou.code,teav.value\
